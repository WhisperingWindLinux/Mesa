...
curl -L --retry 4 -f --retry-all-errors --retry-delay 60 \
  -o "${S3_LLVM_ARTIFACT_NAME}.tar.zst" "https://${S3_HOST}/${S3_ANDROID_BUCKET}/${CI_PROJECT_PATH}/${S3_LLVM_ARTIFACT_NAME}.tar.zst"
tar --zstd -xf "${S3_LLVM_ARTIFACT_NAME}.tar.zst"
popd

LLVM_INSTALL_PREFIX="${DEST_DIR}/${S3_LLVM_ARTIFACT_NAME}"
