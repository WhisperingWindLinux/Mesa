...
.. code-block:: sh
   ...
   if [ -z "$ANDROID_NDK_ROOT" ]; then
     echo "ERROR: script needs ANDROID_NDK_ROOT variable."
     exit 1;
   fi

   if [ -z "$ANDROID_SDK_VERSION" ]; then
     echo "ERROR: script needs ANDROID_SDK_VERSION variable."
     exit 1;
   fi

   if [ -z "$LLVM_INSTALL_PREFIX" ]; then
     echo "ERROR: script needs LLVM_INSTALL_PREFIX variable."
     exit 1;
   fi
