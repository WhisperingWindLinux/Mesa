...
* ``PIPE_CAP_ASTC_DECODE_MODE``: Whether the driver supports ASTC decode precision. The :gl-ext:`EXT_texture_compression_astc_decode_mode` extension will only get exposed if :gl-ext:`KHR_texture_compression_astc_ldr` is also supported.
