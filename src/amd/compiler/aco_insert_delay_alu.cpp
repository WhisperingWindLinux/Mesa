...
gen_alu(Instruction* instr, delay_ctx& ctx)
{
   ...
   if (is_trans || is_valu || instr->isSALU()) {
      ...
      if (is_trans) {
         ...
         lanemask = instr->opcode != aco_opcode::v_readlane_b32_e64 &&
      ...
      for (Definition& def : instr->definitions) {
         if (!lanemask || def.regClass() != ctx.program->lane_mask)
...
insert_delay_alu(Program* program)
{
   ...
   for (unsigned i = 0; i < program->blocks.size(); i++) {
      ...
      /* Reset ctx if there is an jump, assume ALU will be done
       * because branch latency is pretty high.
       */
      if (current.linear_succs.size() == 0 ||
