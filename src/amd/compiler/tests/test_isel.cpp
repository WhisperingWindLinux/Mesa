...
BEGIN_TEST(isel.cf.uniform_if_branch_use)
   ...
   nir_push_loop(nb);
   {
      ...
      //>> BB17
      //! /* logical preds: BB15, BB16, / linear preds: BB15, BB16, / kind: uniform, continue, allow_repair_phis, */
   }
   nir_pop_loop(nb, NULL);
...
BEGIN_TEST(isel.cf.hidden_break_no_lcssa_header_phi)
   ...
   nir_loop *loop = nir_push_loop(nb);
   {
      ...
      nir_push_loop(nb);
      {
         ...
         nir_pop_if(nb, NULL);
         //>> BB8
         //! /* logical preds: BB6, BB7, / linear preds: BB6, BB7, / kind: uniform, continue_or_break, allow_repair_phis, */
      }
      nir_pop_loop(nb, NULL);
      //>> BB11
      ...
      //>> BB17
      //! /* logical preds: BB15, BB16, / linear preds: BB15, BB16, / kind: uniform, continue_or_break, allow_repair_phis, */
...
BEGIN_TEST(isel.cf.empty_exec.uniform_if)
   ...
   nir_push_if(nb, nir_unit_test_uniform_amd(nb, 1, 1, .base = 0));
   {
      //>> BB1
      //>> s2: %_ = p_unit_test 1
      //>> p_demote_to_helper %_
      //>> p_unit_test 2, %_
      nir_demote_if(nb, nir_unit_test_divergent_amd(nb, 1, 1, .base = 1));
...
BEGIN_TEST(isel.cf.empty_exec.divergent_if)
   ...
   nir_push_if(nb, nir_unit_test_divergent_amd(nb, 1, 1, .base = 0));
   {
      ...
      nir_push_if(nb, nir_unit_test_divergent_amd(nb, 1, 1, .base = 3));
      {
         ...
         nir_demote_if(nb, nir_unit_test_divergent_amd(nb, 1, 1, .base = 5));
      ...
      nir_push_else(nb, NULL);
      {
         ...
         nir_demote_if(nb, nir_unit_test_divergent_amd(nb, 1, 1, .base = 8));
      ...
      //>> /* logical preds: BB1, / linear preds: BB1, / kind: uniform, */
      //>> BB16
      //! /* logical preds: BB14, BB15, / linear preds: BB14, BB15, / kind: uniform, allow_repair_phis, */
 ...
 * }
 */
BEGIN_TEST(isel.cf.empty_exec.loop_demote)
   if (!setup_nir_cs(GFX11))
      return;
   ...
   nir_push_loop(nb);
   {
      ...
      //>> BB7
      //! /* logical preds: BB5, BB6, / linear preds: BB5, BB6, / kind: uniform, continue_or_break, allow_repair_phis, */
   }
   nir_pop_loop(nb, NULL);
...
BEGIN_TEST(isel.cf.empty_exec.loop_continue_then_break)
   ...
   nir_push_loop(nb);
   {
      ...
      //>> BB20
      //! /* logical preds: BB18, BB19, / linear preds: BB18, BB19, / kind: uniform, continue, allow_repair_phis, */
   }
   nir_pop_loop(nb, NULL);
   //>> BB21
 ...
 * }
 */
BEGIN_TEST(isel.cf.empty_exec.demote_then_uniform_if)
   if (!setup_nir_cs(GFX11))
      return;
   ...
   nir_push_if(nb, nir_unit_test_divergent_amd(nb, 1, 1, .base = 0));
   {
      ...
      nir_demote_if(nb, nir_unit_test_divergent_amd(nb, 1, 1, .base = 2));
      //>> s2: %_ = p_cbranch_z %0:exec rarely_taken
 ...
 * }
 */
BEGIN_TEST(isel.cf.empty_exec.demote_then_divergent_if)
   if (!setup_nir_cs(GFX11))
      return;
   ...
   nir_push_if(nb, nir_unit_test_divergent_amd(nb, 1, 1, .base = 0));
   {
      ...
      nir_pop_if(nb, NULL);
      //>> BB7
      //! /* logical preds: BB2, BB5, / linear preds: BB5, BB6, / kind: uniform, merge, */
      //>> s2: %_ = p_cbranch_z %0:exec rarely_taken
 ...
 * }
 */
BEGIN_TEST(isel.cf.empty_exec.demote_then_loop)
   if (!setup_nir_cs(GFX11))
      return;
   ...
   nir_push_if(nb, nir_unit_test_divergent_amd(nb, 1, 1, .base = 0));
   {
      ...
      nir_push_loop(nb);
      {
         ...
         nir_push_loop(nb);
         {
            ...
            //>> s2: %_ = p_unit_test 3
            //>> s2: %_ = p_cbranch_z %0:exec rarely_taken
            nir_demote_if(nb, nir_unit_test_divergent_amd(nb, 1, 1, .base = 3));
            ...
            //! /* logical preds: BB5, / linear preds: BB5, / kind: uniform, */
            //>> BB11
            //! /* logical preds: BB9, BB10, / linear preds: BB9, BB10, / kind: uniform, break, allow_repair_phis, */
            //! v1: %vgpr1 = p_phi %vgpr0, v1: undef
         ...
         //! v1: %vgpr2 = p_phi %vgpr0, %vgpr1
         //! s1: %sgpr2 = p_linear_phi %sgpr0, %sgpr1
         //>> BB13
         //! /* logical preds: BB3, / linear preds: BB3, / kind: uniform, */
         //>> BB14
         //! /* logical preds: BB12, BB13, / linear preds: BB12, BB13, / kind: uniform, allow_repair_phis, */
         //! v1: %vgpr3 = p_phi %vgpr2, v1: undef
         ...
         //>> BB20
         //! /* logical preds: BB18, BB19, / linear preds: BB18, BB19, / kind: uniform, continue_or_break, allow_repair_phis, */
      }
      nir_pop_loop(nb, NULL);
      //>> BB23
      ...
      //! /* logical preds: BB1, / linear preds: BB1, / kind: uniform, */
      //>> BB25
      //! /* logical preds: BB23, BB24, / linear preds: BB23, BB24, / kind: uniform, allow_repair_phis, */
      //! s1: %sgpr5 = p_linear_phi %sgpr4, s1: undef
      //! v1: %vgpr4 = p_phi %vgpr3, v1: undef
   }
   nir_pop_if(nb, NULL);
   //>> BB27
   //! /* logical preds: / linear preds: BB25, BB26, / kind: invert, */
   //! s1: %sgpr6 = p_linear_phi %sgpr5, s1: undef
   //>> BB30
   //! /* logical preds: BB25, BB28, / linear preds: BB28, BB29, / kind: uniform, top-level, merge, */
   //! v1: %vgpr = p_phi %vgpr4, v1: undef
   //! s1: %sgpr = p_linear_phi %sgpr6, %sgpr6
