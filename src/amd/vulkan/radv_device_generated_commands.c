...
#define PKT3_INDIRECT_BUFFER_SIZE 16
...
{
   return radv_pad_cmdbuf(device, PKT3_INDIRECT_BUFFER_SIZE, ip_type);
}
                      ...
                      struct dgc_cmdbuf_layout *layout)
{
   ...
   layout->main_size =
      radv_pad_cmdbuf(device, (layout->main_cmd_stride * sequences_count) + PKT3_INDIRECT_BUFFER_SIZE, AMD_IP_GFX);
   layout->ace_size =
      radv_pad_cmdbuf(device, (layout->ace_cmd_stride * sequences_count) + PKT3_INDIRECT_BUFFER_SIZE, AMD_IP_COMPUTE);
...
dgc_cmd_buf_size(nir_builder *b, nir_def *sequence_count, bool is_ace, const struct radv_device *device)
{
   ...
   nir_def *size = nir_iadd_imm(b, nir_imul(b, cmd_buf_stride, sequence_count), PKT3_INDIRECT_BUFFER_SIZE);
                      ...
                      const struct radv_device *device)
{
   ...
   nir_push_if(b, nir_ieq_imm(b, global_id, 0));
   {
      ...
      cmd_buf_size = nir_iadd_imm(b, cmd_buf_size, -PKT3_INDIRECT_BUFFER_SIZE);
                         ...
                         const struct radv_device *device)
{
   ...
   nir_push_if(b, nir_ieq_imm(b, global_id, 0));
   {
      ...
      const uint32_t pad_size = trailer_size - PKT3_INDIRECT_BUFFER_SIZE;
                          ...
                          const struct radv_device *device)
{
   ...
   nir_push_if(b, nir_iand(b, nir_ieq_imm(b, global_id, 0), use_preamble));
   {
      ...
      const uint32_t pad_size = preamble_size - PKT3_INDIRECT_BUFFER_SIZE;
      const uint32_t pad_size_dw = pad_size >> 2;
