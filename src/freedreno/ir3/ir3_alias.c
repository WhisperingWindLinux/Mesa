...
struct alias_table_state {
    ...
    * includes both alias registers as well as GRPs that are reused.
    */
...
{
   for (unsigned reg = FIRST_ALIAS_REG; reg < GPR_REG_SIZE - num_aliases;
        reg++) {
            ...
            unsigned first_src_n)
{
   ...
   foreach_src_in_alias_group_n (alias, alias_n, instr, first_src_n) {
      if (reg_gpr(alias) && !BITSET_TEST(*alloc_regs, alias->num)) {
