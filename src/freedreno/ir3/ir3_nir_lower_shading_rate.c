                            ...
                            UNUSED void *cb_data)
{
   ...
   nir_variable *lut = nir_local_variable_create(
      b->impl, glsl_array_type(glsl_uint_type(), 32, 0),
      "hw_to_vk_shading_rate_lut");
   nir_deref_instr *deref = nir_build_deref_var(b, lut);

   for (uint32_t i = 0; i < 32; i++) {
      nir_deref_instr *element =
         nir_build_deref_array(b, deref, nir_imm_int(b, i));
      nir_build_store_deref(b, &element->def,
                            nir_imm_int(b, hw_to_vk_shading_rate_lut[i]), 0x1);
   }

   nir_deref_instr *result = nir_build_deref_array(b, deref, &intr->def);
                                 ...
                                 UNUSED void *cb_data)
{
   ...
   for (uint32_t i = 0; i < 32; i++) {
      nir_deref_instr *element =
         nir_build_deref_array(b, deref, nir_imm_int(b, i));
      nir_build_store_deref(b, &element->def,
                            nir_imm_int(b, vk_to_hw_shading_rate_lut[i]), 0x1);
   }

   nir_deref_instr *result = nir_build_deref_array(b, deref, intr->src[0].ssa);
   nir_def *r = nir_build_load_deref(b, 1, 32, &result->def, 0);
