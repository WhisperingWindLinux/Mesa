...
static unsigned int
get_swizzle_split(struct radeon_compiler *c, struct rc_swizzle_split *split,
                  struct rc_instruction *inst, unsigned src, unsigned *usemask)
{
   *usemask = 0;
...
static void
rewrite_source(struct radeon_compiler *c, struct rc_instruction *inst, unsigned src)
{
 ...
 */
static unsigned
try_rewrite_constant(struct radeon_compiler *c, struct rc_src_register *reg)
{
   ...
   while (!found_swizzle && !all_inline) {
      ...
      if (swz1 == RC_SWIZZLE_W || swz1 == RC_SWIZZLE_UNUSED || swz1 == RC_SWIZZLE_ZERO ||
          swz1 == RC_SWIZZLE_HALF || swz1 == RC_SWIZZLE_ONE) {
   ...
   if (is_swizzle_inline_constant(swz3)) {
      ...
      for (chan = 0; chan < 3; chan++) {
          ...
          */
         if (old_swz == swz3) {
            SET_SWZ(new_swizzle, 3, GET_SWZ(new_swizzle, chan));
            break;
         }
   ...
   for (chan = 0; chan < 4; chan++) {
      ...
      case RC_SWIZZLE_ONE:
         ...
         break;
      default:
         imms[new_swz] = rc_get_constant_value(c, reg->Index, reg->Swizzle, reg->Negate, chan);
      }
      SET_SWZ(reg->Swizzle, chan, new_swz);
   }
   reg->Index = rc_constants_add_immediate_vec4(&c->Program.Constants, imms);
 ...
 */
static void
clear_channels(struct rc_instruction *inst, unsigned writemask)
{
...
static bool
try_splitting_single_channel(struct radeon_compiler *c, struct rc_instruction *inst)
{
   for (unsigned chan = 0; chan < 3; chan++) {
      ...
      const struct rc_opcode_info *opcode = rc_get_opcode_info(new_inst->U.I.Opcode);
...
static bool
try_splitting_instruction(struct radeon_compiler *c, struct rc_instruction *inst)
{
    ...
    */
   if (inst->U.I.DstReg.WriteMask == RC_MASK_XYZW || inst->U.I.DstReg.WriteMask == RC_MASK_XYZ) {
...
void
rc_dataflow_swizzles(struct radeon_compiler *c, void *user)
{
   ...
   for (inst = c->Program.Instructions.Next; inst != &c->Program.Instructions; inst = inst->Next) {
      ...
      for (src = 0; src < opcode->NumSrcRegs; ++src) {
         ...
         if (!c->is_r500 && c->Program.Constants.Count < R300_PFS_NUM_CONST_REGS &&
