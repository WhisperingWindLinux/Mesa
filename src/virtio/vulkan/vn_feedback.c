                       ...
                       enum vn_feedback_type type)
{
   ...
   const VkAllocationCallbacks *alloc = pool->alloc;
   struct vn_feedback_buffer *feedback_buf;
   uint32_t offset;
...
vn_feedback_cmd_record_internal(VkCommandBuffer cmd_handle,
                                ...
                                VkResult status)
{
   VkMemoryBarrier barrier_before = {
   ...
   /* make pending writes available like fence signal operation */
   if (slot->type == VN_FEEDBACK_TYPE_FENCE)
      barrier_before.srcAccessMask |= VK_ACCESS_MEMORY_WRITE_BIT;
                             ...
                             VkResult status)
{
   ...
   if (ev->feedback_slot) {
      /* TODO use VkBufferMemoryBarrier instead */
...

{
   ...
   result = vn_BeginCommandBuffer(cmd_handle, &begin_info);
   if (result != VK_SUCCESS)
      return result;

   vn_feedback_cmd_record_internal(
      cmd_handle, slot, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_SUCCESS);
